...
Download cactus_dashboard.html
Open in any modern browser (Chrome, Safari, Firefox, Edge) — just double-click
Interact — click tabs to filter by location, hover charts to inspect values, click location cards to drill down
...
Ingestion — reads raw Cactus_2025.xlsx, promotes first row to header, flattens nested transaction/line-item structure
Validation — applies data quality rules (gross ≈ net reconciliation, completeness checks on stock codes and net amounts)
Aggregation — generates monthly scorecard with:
...
Output — writes Cactus_2025_DQ_scorecard.xlsx for audit/compliance
...
Replace source file with the new data export
Run the notebook to generate a fresh data quality scorecard:

bash   jupyter notebook customer_data_pipeline.ipynb

Recompute dashboard data — regenerate the aggregated metrics
Rebuild the HTML dashboard with updated data points
